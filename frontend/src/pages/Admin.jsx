...
const Admin = () => {
  ...
  const handleApproveUser = async (userId, status) => {
    try {
      ...
      setSuccessMessage(`Bruker ${status === 'approved' ? 'godkjent' : 'avvist'}!`);
      ...
      setError(`Greide ikke ${status === 'approved' ? 'godkjenne' : 'avvise'} bruker`);
      console.error(err);
    }
  };
  ...
  const handleProductFormSuccess = () => {
    setSuccessMessage(`Vare ${editingProduct ? 'oppdatert' : 'opprettet'} uten problemer!`);
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex gap-4 mb-8 flex-wrap">
          ...
          <button
            ...
            style={activeTab === 'orders' ? { background: 'linear-gradient(135deg, #14b8a6, #10b981)' } : {}}
          >
            📦 Bestillinger
          </button>
        </div>
          ...
          <>
            {loading ? (
              <div className="container-gradient text-center py-12 animate-fadeIn">
                ...
                <p className="text-gray-700 font-bold">Later brukere til godkjenning...</p>
              </div>
            ) : pendingUsers.length === 0 ? (
              <div className="container-gradient text-center py-12 animate-fadeIn">
                <p className="text-gray-700 text-lg font-bold">✅ Ingen brukere til godkjenning for øyeblikket.</p>
              </div>
            ) : (
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex-1">
                        ...
                        <p className="mt-2 text-xs text-gray-500">
                          Registrert: {new Date(user.joinedDate).toLocaleDateString()}
                        </p>
                      </div>
            ...
            {showProductForm ? (
              ...
              <>
                ...
                {loading ? (
                  ...
                  <div className="container-gradient text-center py-12 animate-fadeIn">
                    <p className="text-gray-700 text-lg font-bold">📦 Ingen varer i butikken enda. Trykk på "Legg til vare" for å starte!</p>
                  </div>
                ) : (
                      ...
                      <div key={product._id} className="container-gradient hover:shadow-2xl transition-all transform hover:scale-105">
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-2xl font-bold text-gray-800">
                            {product.currency === 'NOK' ? 'kr ' : product.currency === 'USD' ? '$' : product.currency === 'EUR' ? '€' : '£'}
                            {product.price.toFixed(2)}
                          </span>
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                            product.isAvailable ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                          }`}>
                            {product.isAvailable ? '✅ Tilgjengelig' : '❌ Ikke tilgjengelig'}
                          </span>
                        </div>
        ...
        {activeTab === 'orders' && (
          <>
            {loading ? (
              <div className="container-gradient text-center py-12 animate-fadeIn">
                <div className="animate-spin rounded-full h-12 w-12 mx-auto mb-4" style={{ border: '4px solid rgba(255,255,255,0.3)', borderTopColor: '#14b8a6' }}></div>
                <p className="text-gray-700 font-bold">Laster inn varer...</p>
              </div>
            ) : orders.length === 0 ? (
              <div className="container-gradient text-center py-12 animate-fadeIn">
                <p className="text-gray-700 text-lg font-bold">📦 Ingen bestillinger enda.</p>
              </div>
            ) : (
              <div className="grid gap-6 animate-fadeIn">
                {orders.map((order) => (
                  <div key={order._id} className="container-gradient hover:shadow-2xl transition-all">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                      <div>
                        <h4 className="font-bold text-gray-700 mb-2">Bestillingsdetaljer</h4>
                        <p className="text-gray-700"><strong>Navn:</strong> {order.customerName}</p>
                        <p className="text-gray-700"><strong>Epost:</strong> {order.customerEmail}</p>
                        {order.customerPhone && (
                          <p className="text-gray-700"><strong>Telefon:</strong> {order.customerPhone}</p>
                        )}
                        {order.user && (
                      ...
                      <div>
                        {order.deliveryAddress && (
                          <div className="mb-2">
                            <h4 className="font-bold text-gray-700 mb-1">Adresse</h4>
                            <p className="text-gray-700 text-sm">{order.deliveryAddress}</p>
                          </div>
                        )}
                        {order.notes && (
                          <div>
                            <h4 className="font-bold text-gray-700 mb-1">Notater</h4>
                    ...
                    <div className="mb-4">
                      <h4 className="font-bold text-gray-700 mb-2">Bestillingsgjenstander</h4>
                      ...
                      <div className="mt-3 p-3 rounded-xl text-right" style={{ background: 'linear-gradient(135deg, rgba(102, 126, 234, 0.1), rgba(118, 75, 162, 0.1))' }}>
