...
const Meetings = () => {
  ...
  const handleRSVP = async (meetingId) => {
    try {
      ...
      setMeetings(
        meetings.map((m) => (m._id === meetingId ? data.meeting : m)),
      );
      ...
      toast.error(
        error.response?.data?.message || "Greide ikke RSVP til møtet",
      );
    }
  };
              ...
              <>
                ...
                >
                  ...
                  <p className="text-gray-700 max-w-3xl mx-auto leading-relaxed text-lg">
                    Bokklubben vår er en fantastisk plass for alle oss
                    bokelskere til å diskutere både månedens bok og alle andre
                    bøker vi kunne ønske å diskutere! Her er det mulighet for å
                    snakke om bøker du elsker, bøker du hater og alle andre
                    følelser du har rundt bøkene du har eller vil lese. Skal vi
                    se på historikken, er dette også en fantastisk arena for å
                    bli oppdatret på gjenens gossip og bare kose deg med litt
                    yapping.
                  </p>
                </div>
              </>
